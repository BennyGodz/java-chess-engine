 ...
 *
 * The OpeningBook contains the actual opening lines.
 *
 * This manager does not randomly choose between all matching
 * moves anymore. Instead, it gives priority to stronger
 * opening lines and only uses randomness when moves have
 * similar priority.
 *
 * The goal is to keep the engine varied while strongly
 * preferring good opening moves.
 */
public class OpeningManager {
     ...
     *
     * Instead of randomly selecting from every matching line,
     * moves are scored according to the quality of the opening
     * line they belong to.
     *
     * A small amount of randomness is kept between moves that
    ...
    public Move getOpeningMove(Board board) {
         ...
         */
        List<PrioritizedMove> candidates =
                new ArrayList<>();
         ...
         */
        List<PrioritizedMove> strongMoves =
                new ArrayList<>();
     ...
     *
     * The longer an opening line continues after the current
     * position, the more established that line is considered.
     *
     * This means:
     *
     * e4
     *
     * from a long well defined line gets more priority than
     * a move that only appears in a very short line.
     */
    private int getMovePriority(
            Board board,
            Move move,
            List<List<String>> matchingOpenings
    ) {
        ...
        for (List<String> opening :
            ...
            /*
             * Longer established lines get more priority.
             *
             * Cap this so extremely long lines do not
            ...
            /*
             * Prefer central pawn moves early.
             *
             * This is only a tie breaker.
             */
            if (isEarlyGame()) {

                if (moveSan.equals("e4")
                        || moveSan.equals("d4")) {

                    priority += 15;
                }

                if (moveSan.equals("c4")
                        || moveSan.equals("Nf3")) {
             ...
             */
            if (isEarlyGame()
                    && moveSan.startsWith("Q")) {

                priority -= 20;
            }

            /*
             * Discourage repeated early knight moves.
             *
             * This specifically helps prevent the engine from
             * constantly choosing lines such as Nc6 / Nf6 merely
             * because they appear in many opening lines.
             */
            if (isEarlyGame()
                    && isKnightMove(moveSan)) {
    ...
    /**
     * Returns every opening line that matches
    ...
    /**
     * Checks whether an opening contains
     ...
     *
     * Clears all previously recorded moves and enables

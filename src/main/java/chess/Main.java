...
public class Main {
    ...
    while (true) {
      ...
      System.out.println(
          "Move "
              ...
              + ": "
              + board.getGameStatus()); /* * Automatic game ending. */
      ...
      try {
        ...
        board.playMove(
            move); /* * The OpeningManager itself determines * whether the current position can continue * the opening. */
   ...
   * Make an engine move.
   *
   * <p>The engine first checks whether there is a book move.
   *
   * <p>If there is:
   *
   * <p>1. Search the position normally. 2. Search the position after the book move. 3. Compare the
   * two. 4. Play the book move only if it is safe.
   *
   * <p>This prevents weird opponent moves from forcing the engine into bad opening moves.
   */
  private static void makeEngineMove(
    ...
    if (normalBestMove == null) {
      ...
      System.out.println("Playing opening move.");
      System.out.println("Opening: " + openingManager.getOpeningName());
      System.out.println("Move: " + board.formatMove(bookMove));
      return;
    }
    ...
    if (difference <= MAX_OPENING_LOSS_CP) {
      ...
      System.out.println("Opening: " + openingManager.getOpeningName());
      System.out.println("Opening move accepted.");
      return;
    }
  ...
  private static Move findEquivalentMove(Board board, Move original) {
    ...
    for (Move move : legalMoves) {
      if (!move.getStart().equals(original.getStart())) {
        continue;
      }
      if (!move.getEnd().equals(original.getEnd())) {
        continue;
      }
      if (move.isPromotion() != original.isPromotion()) {
        continue;
      }
      if (move.isPromotion()) {
        if (move.getPromotionPiece().getNotationSymbol()
            != original.getPromotionPiece().getNotationSymbol()) {
          continue;
        }
      }
      return move;
  ...
  private static Move parseMove(Board board, String rawInput) {
    ...
    if (promotionIndex >= 0) {
      if (promotionIndex != core.length() - 2) {
        return null;
      }
    ...
    if (!core.matches("[KQRBN]?[a-h1-8]{0,2}x?[a-h][1-8]")) {
      return null;
    }
    ...
    for (Move move : legalMoves) {
      ...
      if (piece == null || piece.getNotationSymbol() != pieceLetter) {
        continue;
      }
      boolean isCapture = move.isEnPassant() || board.getPiece(move.getEnd()) != null;
      if (captureSpecified && !isCapture) {
        continue;
      }
      if (move.isPromotion()) {
        if (promotion == '\0') {
        ...
        if (move.getPromotionPiece().getNotationSymbol() != promotion) {
          continue;
        }
      } else if (promotion != '\0') {
        continue;
      }
      if (!matchesDisambiguation(move, disambiguation)) {
        continue;
      }
      matches.add(move);
    }
    ...
    if (suppliedMate && !actual.endsWith("#")) {
      return null;
    }

    if (suppliedCheck && !(actual.endsWith("+") || actual.endsWith("#"))) {
      return null;
    }
  ...
  private static Move parseCoordinateMove(Board board, String input, List<Move> legalMoves) {
    ...
    for (Move move : legalMoves) {
      ...
      if (move.isPromotion()) {
        if (promotion == 'Q' && move.getPromotionPiece() instanceof Queen) {
          return move;
        }
        if (promotion == 'R' && move.getPromotionPiece() instanceof Rook) {
          return move;
        }
        if (promotion == 'B' && move.getPromotionPiece() instanceof Bishop) {
          return move;
        }
        if (promotion == 'N' && move.getPromotionPiece() instanceof Knight) {
          return move;
        }
      } else if (promotion == '\0') {
        return move;
  ...
  private static Move uniqueEndMove(List<Move> legalMoves, int endColumn) {
    ...
    for (Move move : legalMoves) {
      if (move.isCastling() && move.getEnd().getColumn() == endColumn) {
        return move;
      }
    }
